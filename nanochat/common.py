...
def autodetect_device_type():
    # If we're on a TPU VM with PJRT, prefer XLA
    if os.environ.get("PJRT_DEVICE") == "TPU" and xm is not None:
        device_type = "xla"
...
def compute_init(device_type="cuda"):  # cuda|cpu|mps
    ...
    if device_type == "xla":
        assert xm is not None, "device_type 'xla' requires torch_xla to be installed"
    ...
    if ddp and device_type == "cuda":
        ...
        # single-process XLA device (we'll start with xla:0 only)
